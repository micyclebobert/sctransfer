    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            if mean:
             ...
             #   print(nonmissing_indicator)
                if self.masking:
                    final = tf.divide(tf.reduce_sum(final * nonmissing_indicator), nelem)
                else:
...
## has the option to add a ridge penalty on pi
class ZINB(NB):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            if mean:
                nonmissing_indicator = self.nonmissing_indicator
                if self.masking:
                    result = _reduce_mean(result * nonmissing_indicator)
                else:
